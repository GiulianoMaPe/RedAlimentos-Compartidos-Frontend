...
import { listarHistorialPorPuesto } from '@/api/donaciones';
...
export default function HistorialScreen() {
  ...
  const cargarHistorial = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setDonaciones(await listarHistorialPorPuesto(usuario.puesto_id));
    } catch (error) {
  ...
  const estadoIcono = (estado: string) => {
    switch (estado) {
      case 'Recogido': return 'checkmark-circle';
    ...
    <View style={styles.container}>
      {loading ? (
        ...
        <FlatList
          ...
          refreshing={loading}
          ListEmptyComponent={
            <Text style={styles.emptyText}>No hay donaciones finalizadas aún</Text>
          }
        />
      )}
